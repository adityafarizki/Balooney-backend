...
describe(".addPlayer", () => {
    ...
    describe("given the game is still initiating", () => {
        ...
        it("registers new player and update screen state", done => {
            ...
            let screenSendable = { send: (msg: string) => {
    ...
    describe("given the game is already running", () => {
        beforeEach(() => {
            gameController.gameState = 'running';
        })

        it("reject player registration and return error 403", done => {
            let controller = new WebSocket("wss://localhost:8080")
            controller.onopen = () => {
                let msg = {
                    action: "add_player",
                    content: { name: "test" }
...
describe(".handleMessage", () => {
    describe("given invalid message action", () => {
        let msg = {
        ...
        it("rejects the message with status 400", done => {
            let controller = new WebSocket("wss://localhost:8080")
            controller.onopen = () => {
                controller.send(JSON.stringify(msg))
            }
            controller.onmessage = (event) => {
                let msgData = JSON.parse(event.data)
                expect(msgData.statusCode).toEqual(400)
                done()
            }
        })
    })
